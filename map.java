...
public class map {
    ...
    //jens an ra bayad az noe ras beszid ke yek noghte ast
    ras get_location(int number){
        return rases.get(number-1);
    }
    //shoma ras(location) ra midahid shomare ra migirid
    ...
    //vorodi in do tabe do ras ya haman location mored nazar shomast
    //in tabe be shoma yek kelas masir midahad
    path show_pass(ras mabda,ras maghsad){
        path p=new path(mabda,maghsad,rasn);
        p.best_path();
        p.show_path();
        return p;
    }
